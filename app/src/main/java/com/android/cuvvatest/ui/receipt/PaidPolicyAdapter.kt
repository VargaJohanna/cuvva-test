...
class PaidPolicyAdapter(
    ...
    inner class PaidPolicyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(policy: PaidPolicy) {
            itemView.apply {
                ...
                val englishDateTime =
                    date.format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.MEDIUM).withLocale(Locale.UK))
        ...
        private fun setValues(item: TextView, value: Float) {
            if (value >= 0) {
                item.text =
                    String.format(item.context.getString(R.string.receipt_value_positive), "%.2f".format(value / 100))
            } else {
                item.text = String.format(
                    item.context.getString(R.string.receipt_value_negative),
                    "%.2f".format(value / 100 * -1)
                )
            }
        }
    }
    ...
    fun updateList(newList: List<PaidPolicy>) {
        val diffResult = DiffUtil.calculateDiff(PaidPolicyDiffUtilCallback(policyList, newList))
